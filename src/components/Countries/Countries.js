...
const Countries = (props) => {
    ...
    let countriesContent = (countriesCtx.countries.slice(0, 41).map(data => (
        <Country onClick={clickHandler} name={data.name} key={data.name}
    ...
    if (countriesCtx.queried) {
        countriesContent = countriesCtx.queriedCountries.slice(0, 41).map(data => (
            <Country onClick={clickHandler} name={data.name} key={data.name}
                flag={data.flag} population={data.population.toLocaleString()}
                capital={data.capital} region={data.region} />
        ))
